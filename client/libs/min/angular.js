  ...
  angularModule('ng', ['ngLocale'], ['$provide',
    function ngModule($provide) {
      ...
      $provide.provider({
        ...
        $$remote : $$RemoteProvider//plugin for $remote [json liu]
      });
    }
  ]);
}
  ...
  function createInternalInjector(cache, factory) {
    ...
    function invoke(fn, self, locals){
      var args = [],
          ...
          length, i,
          key,
          $remote, $http;//defind for $remote plugin [josn liu]
      ...
      for(i = 0, length = $inject.length; i < length; i++) {
        ...
        //update for $remote plugin [josn liu]
        if (locals && locals.$element && (key === '$remote')) {
			if (!$remote) {
				var $$remote = getService('$$remote');
				//injectRemote(locals.$element, locals, '$remote.', false, $$remote);				
				$remote = locals.$remote = $$remote(locals.$element,false);
				//$http = $remote.$http();
			}
...
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    function Scope() {
      ...
      //update by [json liu] 20150721
      this.$field = function(name) {
			return this.$locale.FIELDS[name] || name;
		};
	  this.$msg = function(name) {
			return this.$locale.MESSAGES[name] || name;
	  }; 
    }
...
/* 
 *$$Remote service [by json liu]
 */
function $$RemoteProvider() {
		var getFailData, errorTag = '$error', callbacks = {} , trsContext="",head = {};
		...
		this.setTrsContext = function(c) {
			trsContext = c;
		};
		
		this.setCommomHead = function(h){
			head = h;
		}
		...
		this.$get = ['$http', '$log', remoteFactory];

		function remoteFactory($http, $log) {
			...
			return function($element, submit, ajaxPort) {
				var view = $element.controller('ngView');
				...
				return {
					...
					get : function(url, data, okTag, errorFn, finalFn, config) {
						return withParams('get', trsContext + url, data, okTag, errorFn, finalFn, config);
					},
					'delete' : function(url, data, okTag, errorFn, finalFn, config) {
						return withParams('delete', trsContext + url, data, okTag, errorFn, finalFn, config);
					},
					head : function(url, data, okTag, errorFn, finalFn, config) {
						return withParams('head', trsContext + url, data, okTag, errorFn, finalFn, config);
					},
					jsonp : function(url, data, okTag, errorFn, finalFn, config) {
						return withParams('jsonp', trsContext + url, data, okTag, errorFn, finalFn, config);
					},
					post : function(url, data, okTag, errorFn, finalFn, config) {
						return withNoParams('post', trsContext + url, data, okTag, errorFn, finalFn, config);
					},
					put : function(url, data, okTag, errorFn, finalFn, config) {
						return withNoParams('put', trsContext + url, data, okTag, errorFn, finalFn, config);
					}
				};

				function withParams(name, url, data, okTag, errorFn, finalFn, config) {
					var params;
					...
					return proxy(url, params, null, okTag, errorFn, finalFn, config, name);
				}

				function withNoParams(name, url, data, okTag, errorFn, finalFn, config) {
					return proxy(url, null, data, okTag, errorFn, finalFn, config, name);
				}

				function proxy(url, params, data, okTag, errorFn, finalFn, config, name) {
					extend(data || {},head);
					...
					var failTag = errorTag, start = Date.now(), http = $http(config), error;
					...
					function success(data, status, headers, config) {
						...
						if (error) {
							//$log.error('remote receive(' + (Date.now() - start) + 'ms): ' + url + ', with error: ' + toJson(error));
							putData(failTag, error, finalFn);
							scope && scope.$root && scope.$root.$broadcast('$remoteError', url, error, status);
						} else {
							//$log.debug('remote receive(' + (Date.now() - start) + 'ms): ' + url + ( data ? ', with data: ' + toJson(data) : ''));
							if (okTag) {
								putData(okTag, data, finalFn);
							} else {
								populate(scope, data, finalFn);
							}
							scope && scope.$root && scope.$root.$broadcast('$remoteSuccess', url, data);
						}
					}
					...
					function fail(data, status, headers, config) {
						...
						if (error) {
							...
							putData(failTag, error, finalFn);
							scope && scope.$root && scope.$root.$broadcast('$remoteError', url, error);
						} else {
							//$log.error('remote receive(' + (Date.now() - start) + 'ms): ' + url + ', with error: ' + status);
							scope && scope.$root && scope.$root.$broadcast('$remoteError', url, error, status);
						}
					}
				...
				function putData(tag, data, finalFn) {
					if (isFunction(tag))
						return tag(data) && finalFn && finalFn(data);
					...
					if (name) {
						...
						populate(s, data, finalFn);
					}
				}

				function populate(scope, data, finalFn) {
					if (isObject(data) && !isArray(data)) {
						for (var key in data)
						if (data.hasOwnProperty(key)) {
							scope[key] = data[key];
						}
						if(isFunction(finalFn)) finalFn(data);
					} else {
